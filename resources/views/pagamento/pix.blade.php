...
<head>
    ...
    <title>{{ __('messages.pay_with_pix') }} – GiftZone</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .valor { font-size: 2.4rem; font-weight: 900; color: white; margin: 16px 0; }
        ...
        .timer { font-family: 'Gasoek One', sans-serif; font-size: 2.8rem; color: var(--accent); letter-spacing: 4px; transition: color .3s; }
        ...
        .qr-wrap { background: white; border-radius: 12px; padding: 16px; display: inline-block; margin-bottom: 24px; }
...
<body>
<div class="card">
    <img class="logo-pix" src="https://upload.wikimedia.org/wikipedia/commons/5/50/Pix_%28Brazil%29_logo.svg" alt="PIX">
    <h1>{{ __('messages.pay_with_pix') }}</h1>
    ...
    <div class="timer-wrap">
        <p class="timer-label">{{ __('messages.time_remaining') }}</p>
    ...
    <p class="copy-label">{{ __('messages.pix_copy_paste') }}</p>
    <div class="copy-wrap">
        <input class="copy-input" id="pixPayload" type="text" value="{{ $payload }}" readonly>
        <button class="btn-copy" id="btnCopy" onclick="copiarPix()">{{ __('messages.copy') }}</button>
    </div>

    <p class="expirado-msg" id="expiradoMsg">{{ __('messages.qr_expired') }}</p>

    <form action="{{ route('pagamento.pix.confirmar') }}" method="POST">
        @csrf
        <button type="submit" class="btn-confirmar" id="btnConfirmar">{{ __('messages.already_paid') }}</button>
    </form>
...
<script>
    ...
    const interval = setInterval(() => {
        ...
        if (remaining <= 60) { timerEl.classList.add('urgente'); fillEl.style.background = '#e85020'; }
        if (remaining <= 0) {

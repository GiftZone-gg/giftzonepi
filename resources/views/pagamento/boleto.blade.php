...
<div class="card">
    ...
    <h1>📄 BOLETO</h1>
    <p class="subtitulo">Pague em qualquer banco ou lotérica</p>

    <div class="valor">
        <span>Valor a pagar</span>
        R$ {{ number_format($total, 2, ',', '.') }}
    </div>
    <p class="vencimento">Vencimento: <strong>{{ $vencimento }}</strong></p>

    <div class="barcode">
        <div class="barcode-lines" id="barras"></div>
    </div>

    <p class="copy-label">Linha Digitável</p>
    <div class="copy-wrap">
    ...
    <div class="aviso">
        ⚠️ <strong>Atenção:</strong> O pagamento pode levar até <strong>3 dias úteis</strong> para ser confirmado. Seu pedido será processado após a compensação.
    </div>

    <form action="{{ route('pagamento.boleto.confirmar') }}" method="POST">
        @csrf
        <button type="submit" class="btn-confirmar">Já paguei — Confirmar pedido</button>
    </form>

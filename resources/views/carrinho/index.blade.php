...
<head>
    ...
    <title>GiftZone - {{ __('messages.cart') }}</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cart-page-title { font-family: 'Gasoek One', sans-serif; font-size: 36px; font-weight: 400; color: #FDE9A2; margin-bottom: 28px; margin-top: 12px; }
        ...
        .cart-wrapper { display: grid; grid-template-columns: 1fr 340px; gap: 28px; align-items: start; }
        .cart-panel { background: #001A20; border-radius: 20px; padding: 28px; }
        .cart-panel-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; }
        .cart-panel-title { font-family: 'Gasoek One', sans-serif; font-size: 18px; font-weight: 400; color: #FDE9A2; text-transform: uppercase; }
        .btn-remove-all { display: inline-flex; align-items: center; gap: 8px; background: rgba(231,76,60,0.15); border: 1px solid rgba(231,76,60,0.4); color: #e74c3c; border-radius: 8px; padding: 8px 16px; font-size: 13px; font-weight: 500; cursor: pointer; text-decoration: none; transition: all 0.2s; }
        .btn-remove-all:hover { background: rgba(231,76,60,0.3); border-color: #e74c3c; }
        .cart-item { display: flex; align-items: center; gap: 20px; padding: 18px 0; border-bottom: 1px solid rgba(255,255,255,0.07); }
        .cart-item:last-child { border-bottom: none; }
        .item-thumb { width: 130px; min-width: 130px; height: 80px; border-radius: 10px; background: #1F6D7E; overflow: hidden; position: relative; }
        .item-thumb-badge { position: absolute; top: 0; left: 0; right: 0; text-align: center; font-size: 9px; font-weight: 700; padding: 4px 0; text-transform: uppercase; letter-spacing: 0.5px; }
        .item-thumb-img { width: 100%; height: 100%; object-fit: cover; }
        .item-info { flex: 1; }
        .item-name { font-family: 'Gasoek One', sans-serif; font-size: 15px; font-weight: 400; color: white; margin-bottom: 4px; }
        .item-platform { font-size: 12px; color: #6a9a94; margin-bottom: 12px; }
        .item-qty-form { display: inline-flex; align-items: center; gap: 8px; }
        .qty-input { width: 56px; background: #002830; border: 1px solid rgba(255,255,255,0.15); border-radius: 8px; color: white; font-size: 14px; text-align: center; padding: 6px 8px; outline: none; transition: border-color 0.2s; }
        .qty-input:focus { border-color: #FFDC74; }
        .btn-qty-update { background: rgba(52,152,219,0.2); border: 1px solid rgba(52,152,219,0.4); color: #3498db; border-radius: 8px; padding: 6px 14px; font-size: 12px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
        .btn-qty-update:hover { background: rgba(52,152,219,0.35); }
        .item-right { display: flex; flex-direction: column; align-items: flex-end; gap: 12px; }
        .item-price { font-family: 'Gasoek One', sans-serif; font-size: 18px; color: #FFDC74; }
        .btn-remove { display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; background: rgba(231,76,60,0.1); border: 1px solid rgba(231,76,60,0.3); border-radius: 8px; cursor: pointer; color: #e74c3c; transition: all 0.2s; }
        .btn-remove:hover { background: rgba(231,76,60,0.25); border-color: #e74c3c; }
        .btn-remove svg { width: 16px; height: 16px; }
        .cart-summary { background: #001A20; border-radius: 20px; padding: 28px; position: sticky; top: 110px; }
        .summary-title { font-family: 'Gasoek One', sans-serif; font-size: 18px; font-weight: 400; color: #FDE9A2; text-transform: uppercase; margin-bottom: 24px; }
        .coupon-area { display: flex; gap: 8px; margin-bottom: 24px; }
        .coupon-input { flex: 1; background: #002830; border: 1px solid rgba(255,255,255,0.12); border-radius: 10px; color: white; font-size: 13px; padding: 10px 14px; outline: none; transition: border-color 0.2s; }
        .coupon-input::placeholder { color: #4a7a74; }
        .coupon-input:focus { border-color: #FFDC74; }
        .btn-coupon { background: #FFDC74; color: #001A20; border: none; border-radius: 10px; padding: 10px 18px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.2s; white-space: nowrap; }
        .btn-coupon:hover { background: #ffe99e; }
        .summary-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.07); font-size: 14px; color: #a0c4bf; }
        .summary-row:last-of-type { border-bottom: none; }
        .summary-total { display: flex; justify-content: space-between; align-items: center; padding: 18px 0 20px; font-family: 'Gasoek One', sans-serif; font-size: 22px; font-weight: 400; color: white; border-top: 1px solid rgba(255,255,255,0.12); margin-top: 8px; }
        .summary-total span:last-child { color: #FFDC74; }
        .btn-finalizar { display: block; width: 100%; background: #FFDC74; color: #001A20; border: none; border-radius: 12px; padding: 16px; font-family: 'Gasoek One', sans-serif; font-size: 18px; font-weight: 400; text-align: center; cursor: pointer; transition: background 0.2s, transform 0.15s; margin-bottom: 12px; }
        .btn-finalizar:hover { background: #ffe99e; transform: translateY(-2px); }
        .btn-continuar { display: block; width: 100%; background: transparent; border: 1px solid rgba(255,255,255,0.2); color: rgba(255,255,255,0.7); border-radius: 12px; padding: 14px; font-size: 14px; font-weight: 500; text-align: center; text-decoration: none; transition: all 0.2s; }
        .btn-continuar:hover { border-color: rgba(255,255,255,0.5); color: white; }
        .empty-cart { text-align: center; padding: 80px 20px; }
        .empty-icon { width: 80px; height: 80px; margin: 0 auto 24px; opacity: 0.3; }
        .empty-cart h2 { font-family: 'Gasoek One', sans-serif; font-size: 24px; font-weight: 400; margin-bottom: 12px; color: #FDE9A2; }
        .empty-cart p { color: #6a9a94; margin-bottom: 28px; font-size: 15px; }
        .btn-ir-catalogo { display: inline-block; background: #FFDC74; color: #001A20; border-radius: 12px; padding: 14px 36px; font-family: 'Gasoek One', sans-serif; font-size: 16px; font-weight: 400; text-decoration: none; transition: background 0.2s, transform 0.15s; }
        .btn-ir-catalogo:hover { background: #ffe99e; transform: translateY(-2px); }
        ...
        @media (max-width: 900px) { .container { padding: 96px 20px 40px; } .cart-wrapper { grid-template-columns: 1fr; } .cart-summary { position: static; } header { padding: 20px 20px; } }
        @media (max-width: 600px) { .item-thumb { width: 80px; min-width: 80px; height: 56px; } .item-name { font-size: 13px; } }
    </style>
</head>
<body>
...
<div class="container">
    <h1 class="cart-page-title">{{ __('messages.my_cart_title') }}</h1>
    ...
    @if(empty($itens))
        <div class="empty-cart">
            ...
            <h2>{{ __('messages.empty_cart_title') }}</h2>
            <p>{{ __('messages.empty_cart_msg') }}</p>
            <a href="{{ route('catalogo') }}" class="btn-ir-catalogo">{{ __('messages.view_catalog') }}</a>
        </div>
    @else
        <div class="cart-wrapper">
            <div class="cart-panel">
                <div class="cart-panel-header">
                    <span class="cart-panel-title">{{ __('messages.my_items') }}</span>
                ...
                <div class="cart-item">
                    <div class="item-thumb">
                        ...
                        @if($item['produto']->imagem_principal)
                            <img class="item-thumb-img" src="{{ asset('images/' . $item['produto']->imagem_principal) }}" alt="{{ $item['produto']->nome }}" onerror="this.style.display='none'">
                        @endif
                    </div>
                    <div class="item-info">
                        ...
                        <form action="{{ route('carrinho.atualizar', $item['produto']->id) }}" method="POST" class="item-qty-form">
                            ...
                            <button type="submit" class="btn-qty-update">{{ __('messages.update') }}</button>
                        </form>
                    </div>
                    <div class="item-right">
                        ...
                        <form action="{{ route('carrinho.remover', $item['produto']->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn-remove" title="{{ __('messages.remove_item') }}">
            ...
            <div class="cart-summary">
                <div class="summary-title">{{ __('messages.summary') }}</div>
                <div class="coupon-area">
                    <input type="text" class="coupon-input" placeholder="{{ __('messages.coupon_placeholder') }}">
                    <button class="btn-coupon">{{ __('messages.apply') }}</button>
                </div>
                ...
                <a href="{{ route('catalogo') }}" class="btn-continuar">{{ __('messages.continue_shopping') }}</a>
            </div>
        </div>
    @endif
</div>
